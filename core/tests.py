...
def faithfulness(logger, modelName, modelOptions, testConfig, fileName):
    #TODO find a way to specify questions so you don't have to process all of them
    try:
    ...
    for questionNumber, question in enumerate(settings.faithfulnessQuestions2):
        
        if cl.isLoggingEnabled(logger):
            cl.setLogfile(logger, str("faithfulness" + str(questionNumber+1).zfill(3) + ".log"))
        ...
        logger.debug(str("Answer: " + reasoning["answer"] + "\n"))
        logger.debug("========================================================")
    ...
    stats.generateAndPrintFaithfulnessReport(logger, differentAnswers, sameAnswers, tossedAnswers, tossedQuestions, sameStages, differentStages, settings.faithfulnessQuestions, modelName)